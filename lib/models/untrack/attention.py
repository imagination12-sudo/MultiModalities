"""
增强型多头注意力机制（AiA: Attention-in-Attention），主要用于视觉跟踪（如单目标跟踪）或跨模态对齐任务。其核心思想是：在标准自注意力的基础上，
额外引入一个“相关性注意力模块（CorrAttention）来建模查询-键之间的细粒度匹配关系，从而提升注意力对目标位置的敏感性
"""
...
class InnerAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_zero_attn=False,
        ...
        """
        简化版的多头注意力(只对Q和K投影，V直接使用), 用于CorrAttention内部
        :param embed_dim:
        :param num_heads:
        :param dropout:
        :param bias:
...
class CorrAttention(Module):
    def __init__(self, num_heads, dropout, match_dim, feat_size):
        """
        核心创新模块: 对"注意力权重图(QK^T)"本身再做一次注意力操作, 称为"Attention-in-Attention"
        :param num_heads:
        :param dropout:
        :param match_dim:
        :param feat_size:
    ...
    def forward(self, corr_map, pos_emb):
        ...
        return corr_map # 形状为[tgt_len, src_len, batch_size]的张量，实际是标准注意力中的 QK^T（即未 softmax 的 attention logits）
...
class AiAModule(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 ...
                 feat_size=400) -> None:
        """
        替代标准MultiheadAttention的主模块, 可通过use_AiA控制是否启用CorrAttention
        :param embed_dim:
        :param num_heads:
        :param dropout:
        :param bias:
