"""
多模态提示学习(Prompt based Mutimodal Fusion):
    解决单目标跟踪（SOT）中的多模态输入问题(如RGB + Depth), 但不直接进行拼接或早期融合：
        1.将Depth 图像视为"提示" (Prompt)
        2.通过可学习的模块 Prompt, 将 Depth 信息注入到RGB特征之中
        3. 保持主干网络结构不变，实现轻量、灵活、可插拔的多模态增强
"""
...
class VisionTransformerP(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        """
        两个独立的 patch_embed, 参数不共享
        输入通道：RGB为"3", Depth为"1"
        """
        ...
        """分离位置编码，不使用全局位置编码, 而是template和search分别学习位置编码"""
        ...
        '''prompt parameters'''
        """
        vipt_shaw: 仅在 第0层 融合Prompt
        vipt_deep: 在每一层(deep)都融合Prompt
        """
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        '''
        if self.prompt_type in ['vipt_shaw', 'vipt_deep']:
            # Step1: 将 tokens 转为feature map(B, C, H, W)
