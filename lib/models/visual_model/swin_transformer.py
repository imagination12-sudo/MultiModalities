...
"""
视觉编码器主干网络
    关键函数：build_swin_transformer_backbone
        1. 构建Swin Transformer主干
        2. 支持冻结部分阶段
        3. 输出多尺度特征
    核心类：SwinTransformer
        1. 包含PatchEmbed、BasicStage、PatchMerging等组件
        2. 实现分层视觉特征提取
"""
...
def build_swin_transformer_backbone(name,
    ...
    elif load_pretrained and 'url' in _cfg[name]:
        pretrained_model_path = _cfg[name]['url']
    # 如果 pretrained_path=None 且 load_pretrained=False，则不加载预训练权重
    ...
    if overwrite_embed_dim is not None:
        ...
        pretrained_model_path = None  # embed_dim 改变后，原权重不兼容，强制不加载

    transformer = SwinTransformer(out_indices=output_layers, frozen_stages=frozen_stages, **params)
    transformer.init_weights(pretrained_model_path)  # 支持本地路径或 None
    return transformer