...
"""
语义引导时间建模模块
    关键构建: build_transformer(cfg)
        1. 返回一个完整的Transformer (含 encoder + decoder)
        2. 但在 JointNLP 中仅使用其 decoder 部分
    实际用途:
        1. 输入：mix_temporal = [text_cls, temporal_roi_feature]
        2. 查询：temporal_query_embed
        3. 输出：时序增强的查询向量
"""

"""
⚠️ 注意：虽然 build_transformer 返回带 encoder 的模型，但在 JointNLT.forward_joint 中，
temporal_decoder 被当作 decoder-only 使用（传入 src=mix_temporal 作为 memory，query=temporal_query）

✅ 对应代码：transformer.py 中的 Transformer + JointNLT.py 中调用 self.temporal_decoder(...)
"""

"""
目标解码器
    关键构建：build_decoder(cfg)
        1. 返回 TransformerDecoder实例
        2. 用于将查询(target_query) 与 搜索区域特征 (search_tokens) 交互
    对应代码：
        transformer.py中的 TransformerDecoder + build_decoder 函数
"""
