...
"""
多源关系建模模块
    实现类: VisionLanguageEncoder
        1. 一个纯 encoder 的 Transformer, 用于融合视觉和语言token
        2. 输入包括: 拼接后的 vl_src (文本+模板+搜索区域)、vl_mask、vl_pos
        3. 使用 TransformerEncoder 和 TransformerEncoderLayer
    构建函数:
        build_vl_transformer(cfg)
"""
