...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, max_seq_len=None, bert_model='bert-base-uncased', bert_path=None):
        """
        args:
        ...
        if p_datasets is None:
            print('p_datasets is none', self.datasets)
            p_datasets = [len(d) for d in self.datasets] #==1
        # Normalize
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                # 读取templat_masks 和 search_masks，如果数据集未提供，则使用全零张量来代替作为占位符
                ...
                # 替换原来的 template_nl = template_anno['nlp'][0]
                if 'nlp' in template_anno and len(template_anno['nlp']) > 0:
                    template_nl = template_anno['nlp'][0]
                else:
                    # 为无语言描述的数据集提供默认文本
                    template_nl = "target object"


                nl_token_ids, nl_token_masks = self.extract_token_from_nlp(template_nl, self.max_seq_len)
