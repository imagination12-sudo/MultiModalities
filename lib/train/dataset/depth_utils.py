...
# 读取并融合 RGB 图像和深度图像，根据 dtype 返回不同格式的输出（如 4通道、6通道、伪彩色等）。
def get_rgbd_frame(color_path, depth_path, dtype='rgbcolormap', depth_clip=False):
    ...
    '''
    # print("depth_path ===========", depth_path, type(depth_path))
    # print("color_path ===========", color_path, type(color_path))
...
# 功能与 get_rgbd_frame 完全一致，只是内部变量名用 dp（代表 "x"，可能是为通用多模态设计，如 Event、Thermal、Depth 等
...
# 存储深度处理和 GrabCut 的超参数，避免硬编码。
class p_config(object):
    grabcut_extra = 50  # GrabCut 矩形框向外扩展的像素数
    grabcut_rz_threshold = 300 # 图像尺寸超过此值时进行下采样
    grabcut_rz_factor = 1.5 # 下采样比例（加速 GrabCut）
    minimun_target_pixels = 16 # 有效目标像素最小数量
    grabcut_iter = 3 # GrabCut 迭代次数
    radius = 500 # 深度层提取时的容忍范围（±500mm）

# 根据目标深度，提取“深度层”图像 —— 只保留 [target_depth ± radius] 范围内的像素，其余设为背景
...
# 去除图像中的小连通区域（“气泡”噪声），常用于 mask 或深度图后处理
...
# 基于目标边界框，使用 GrabCut 分割目标区域，并估计其典型深度值（比直接取中值更鲁棒）
