...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            if self.dataset_name in ['depthtrack_rgbd', 'depthtrack_rgbd_train', 'cdtb', 'votrgbd22_rgbd']:
                image = self._read_rgbd_image(frame_path, dtype='rgbcolormap')  # [H, W, 7]
            else:
    ...
    def _read_rgbd_image(self, image_file: str, dtype='color'):
        if isinstance(image_file, (tuple, list)) and len(image_file) == 2:
            color_path, depth_path = image_file
            img = get_rgbd_frame(color_path,depth_path, dtype=dtype, depth_clip=True)
            modality = img[:, :, 3:]
