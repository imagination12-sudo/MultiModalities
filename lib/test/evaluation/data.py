...
class Sequence:
    ...
    def __init__(self, name, frames, dataset, ground_truth_rect, language_description=None,ground_truth_seg=None, init_data=None,
    ...
    def init_info(self):
        ...
        lang = getattr(self, 'language_description', None)
        if lang is not None:
            info['language_description'] = lang
        return info
    ...
    def get_language_description(self):
        """Return the natural language description of the target."""
        return getattr(self, 'language_description', None)


    def __repr__(self):
        lang = getattr(self, 'language_description', None)
        lang_str = f", lang='{lang[:30]}...'" if lang else ""
        return "{self.__class__.__name__} {self.name}, length={len} frames{lang_str}".format(
            self=self, len=len(self.frames), lang_str=lang_str
        )
